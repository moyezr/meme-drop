...
| AI — tweet analysis | Claude Haiku (Anthropic) |
| AI — meme tagging | GPT-4o Vision (OpenAI) |
| AI — embeddings | text-embedding-3-small (OpenAI, 1536-dim) |
...
6. Backend runs tweet context analysis via Claude Haiku:
...
7. Backend generates embedding from context JSON using `text-embedding-3-small`
...
7. Backend calls GPT-4o Vision **inline** with structured prompt requesting: `{ name, emotion, format_type, use_cases[], example_tweet_contexts[], is_evergreen }`
...
**System tags (auto-generated by GPT-4o Vision on save):**
...
| system_tags | jsonb | Auto-generated from GPT-4o Vision |
...
- `OPENAI_API_KEY` — for embeddings + GPT-4o Vision
- `ANTHROPIC_API_KEY` — for Claude Haiku context analysis
...
5. **Suggestion engine** — Claude Haiku analysis → embedding → pgvector nearest-neighbor → scoring → return results
...
8. **Save flow** — hover save button on images, API call, download to local storage, inline GPT-4o Vision tagging, toast notification
